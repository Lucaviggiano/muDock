...
namespace mudock {
  ...
  struct adt_quant_score: public scoring<queue_type> {
    ...
    void prepare(batch<static_molecule> &batch) {
      ...
      atom_bins.alloc(tot_atoms_in_batch);
      num_nonbond()[0] = 0;
      ...
      for (int ligand_index{0}; ligand_index < batch_ligands; ++ligand_index) {
        ...
        std::vector<int> bins = build_atom_to_bin_map(ligand, thresholds);
        std::memcpy((void *) (atom_bins() + stride_atoms), bins.data(), num_atoms * sizeof(int));

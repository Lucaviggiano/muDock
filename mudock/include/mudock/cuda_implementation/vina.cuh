...
namespace mudock {
    ...
    __device__ inline fp_type gauss1(const size_t idx, const fp_type* __restrict__ dst_mtx, int size) {
        fp_type gauss1 = 0;
        if(dst_mtx[idx] != 0) gauss1 = exp(- pow(dst_mtx[idx] / 0.5, 2));
        return gauss1;
    }

    __device__ inline fp_type gauss2(const size_t idx, const fp_type* __restrict__ dst_mtx, int size) {
        fp_type gauss2 = 0;
        if(dst_mtx[idx] != 0) gauss2 = exp(- pow((dst_mtx[idx] - 3) / 2, 2));
        return gauss2;
    }

    __device__ inline fp_type repulsion(const size_t idx, const fp_type* __restrict__ dst_mtx, int size) {
        return pow((dst_mtx[idx] < 0) * dst_mtx[idx], 2);
    }

    __device__ inline fp_type hydrophobic(const size_t idx, const fp_type* __restrict__ dst_mtx, const int* __restrict__ rec_lig_is_hydrophobic, int size) {
    ...
    __device__ inline fp_type hbonding(const size_t idx, const fp_type* __restrict__ dst_mtx, const int* __restrict__ rec_lig_is_hb, int size) {
    ...
    __device__ inline fp_type score_function(
        ...
        for( size_t i = 0; i < size; i++) {
            ...
            hydro += hydrophobic(idx, dst_mtx, ij_is_hydrophobic, size);
            hbond += hbonding(idx, dst_mtx, ij_is_hbond, size);
        }
    ...
    __device__ inline size_t parse_data(
        /// Protein data
    ...
    __device__ inline size_t parse_intra_data(
        const fp_type* __restrict__ ligand_x,
        const fp_type* __restrict__ ligand_y,
        const fp_type* __restrict__ ligand_z,
        const int* __restrict__ l_is_hbond_acceptor,
